...
//        scene.getStylesheets().add(getClass().getResource("application.css").toExternalForm());
    ...
    private HBox createMenuItem(String text, String iconFileName) {
        ImageView icon = createImageView("/img/" + iconFileName, 30, 30); // Giả định icon nằm trong /icons/
        Label label = new Label(text);
        label.setStyle("-fx-text-fill: white");
        HBox item = new HBox(10, icon, label); // Khoảng cách 10 giữa icon và text
        item.setStyle("-fx-font-weight: bold; -fx-font-size: 15; -fx-font-family: 'Poppins'; -fx-cursor: hand");
...
//        }
    ...
    private HBox createLogoutButton() {
        ImageView icon = createImageView("/img/logout.png", 30, 30); // Giả định icon logout
        Label label = new Label("Đăng xuất");
        label.setStyle("-fx-text-fill: white; -fx-font-weight: bold; -fx-font-size: 20; -fx-font-family: 'Poppins';");
