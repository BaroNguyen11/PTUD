...
public class Gui_QLMon extends BorderPane{
	...
	private HBox createInputField(String labelText, TextField textField, boolean isReadOnly) {
	 ...
	 public void showMonAnModal(MonAn mon) {
		    ...
		    stHinhAnh.getChildren().addAll(boxHinhAnh, imgMonAn);
		    ...
		    // Event cho btnThemAnh: Chọn file ảnh
		    btnThemAnh.setOnAction(e -> {
		    ...
		    btnSua.setOnAction(e ->{
		    		if(btnSua.getText().equals("Sửa")) {
		    			xuLiSuaMon(txtTen, cboLoai, textArea, btnThemAnh);
		    			...
		    			if(xuLiLuuSauKhiSua(txtTen, cboLoai, textArea, btnThemAnh, txtMa.getText())) {
		    				modal.close();
		    			}
		    		}
	 ...
	 public void showMonAnModalThemMon() {
		    ...
		    boxHinhAnh.setStyle("""
		     ...
		     // Tạo ImageView cho preview (ban đầu ẩn)
		        ...
		        // Thêm cả hai vào StackPane (icon chồng lên imgPreview ban đầu)
		    ...
		    vboxTrai.getChildren().addAll(stThemAnh, btnThemAnh);
		    vboxTrai.setAlignment(Pos.CENTER);
	 ...
	 public void xuLiSuaMon(TextField txtTen, ComboBox cboLoai, TextArea textArea, Button btnThemAnh) {
	    ...
	    btnThemAnh.setDisable(false);
	    
	 }
	 
	 public boolean xuLiLuuSauKhiSua(TextField txtTen, ComboBox<String> cboLoai, TextArea textArea, Button btnThemAnh, String ma) {
		String tenMon = txtTen.getText().trim();
