...
//        		banChon = banAn;
	...
	private VBox taoPhanTrai(BanAn banAn, KhachHang khachHang, PhieuDatBan phieuDatBan) {
		...
		for (KhuyenMai khuyenMai : dsKMApDung) {
			System.out.println(khuyenMai.getGiaTriGiam());
		}
		...
		if (dsKMApDung.isEmpty()) {
			...
			tienThanhToan = control.tinhTienThanhToan(parseVNDToDouble(txtTamTinh.getText()), tienCoc,
					kmDau.isGiamGiaPhanTram(), kmDau.getGiaTriGiam(), thue, kmDau.getGiaTriToiDa());
			giamGia = control.tinhTienGiamGia(kmDau.getGiaTriGiam(), kmDau.isGiamGiaPhanTram(),
		...
		cboKM.setOnAction(event -> {
			...
			if (selected != null) {
				double tienTT = control.tinhTienThanhToan(parseVNDToDouble(txtTamTinh.getText()), tienCoc,
						selected.isGiamGiaPhanTram(), selected.getGiaTriGiam(), thue, selected.getGiaTriToiDa());
				double giaGiam = control.tinhTienGiamGia(tamTinh, selected.isGiamGiaPhanTram(),
