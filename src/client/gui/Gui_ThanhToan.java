...
import client.ctrl.ThanhToan_Ctrl;
...
public class Gui_ThanhToan extends BorderPane {
    ...
    private VBox taoPhanTrai(List<PhieuDatBan> dsPhieu) {
        ...
        KhachHang khachHang = new KhachHangClient().getKhachHangById(dsPhieu.get(0).getKhachHang().getMaKhachHang());
...
//    }
    ...
    public void thucHienIn(String maHoaDon, String dsBan) {
        QLHDClient qlhdClient = new QLHDClient();
        if (maHoaDon == null || maHoaDon.isBlank()) {
            showAlert(AlertType.ERROR, "Lỗi", "Vui lòng chọn một hóa đơn trước khi in");
            return;
        }
        List<String> dsMon = qlhdClient.getChiTietHoaDonTheoMa(maHoaDon);
        HoaDon hoaDon = qlhdClient.getHoaDonById(maHoaDon);
    ...
    private VBox taoHeader(int page, int totalPages, HoaDon hoaDon, String dsBan) {
        ...
        NhanVien nv = nhanVienClient.getNhanVienByMa(hoaDon.getNhanVien().getMaNhanVien());
...
//			}
    ...
    private VBox taoFooter(boolean lastPage, HoaDon hoaDon) {
        ...
        String[] hoaDonSplit = new QLHDClient().layHoaDonString(hoaDon.getMaHoaDon()).split(",");
