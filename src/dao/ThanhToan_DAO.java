...
public class ThanhToan_DAO extends MongoDaoSupport {
    ...
    public HoaDon getHoaDonTheoMaBan(String maBan) {
        Document p = col("PhieuDatBan").find(Filters.eq("maBan", maBan)).sort(new Document("thoiGianBatDau", -1)).first();
        return p == null ? null : hoaDon(one("HoaDon", "maHoaDon", s(p, "maHoaDon")));
    }

    public NhanVien getNhanVienByMa(String maNhanVien) { return NhanVien_DAO.getNhanVienByMa(maNhanVien); }

    public KhachHang getKhachHangByMa(String maKhachHang) { return KhachHang_DAO.getKhachHangById(maKhachHang); }

    public List<String> getChiTietHoaDonTheoMa(String maHoaDon) { return new QLHD_DAO().getChiTietHoaDonTheoMa(maHoaDon); }
    ...
    public double getSoTienGiamCaoNhatTheoHoaDon(String maHoaDon) {
        double max = 0;
        for (Document d : docs("ChiTietKMHD", Filters.eq("maHoaDon", maHoaDon))) max = Math.max(max, dbl(d, "soTienGiam"));
        return max;
    }

    public boolean capNhatTrangThaiThanhToan(String maHoaDon, String phuongThuc) {
        return update("HoaDon", Filters.eq("maHoaDon", maHoaDon), new Document("trangThai", "Đã thanh toán").append("phuongThuc", phuongThuc));
    }
    ...
    public List<KhuyenMai> getKhuyenMaiApDungChoHoaDon(String maHoaDon, double tongTien) {
        ...
        for (Document d : docs("KhuyenMai")) {
            ...
            if (km != null && km.getDieuKienApDung() <= tongTien && !today.isBefore(km.getNgayBatDau()) && !today.isAfter(km.getNgayKetThuc())) list.add(km);
        }
        return list;
    }

    public boolean taoChiTietKMHD(String maHoaDon, String maKhuyenMai, double soTienGiam) {
        col("ChiTietKMHD").insertOne(new Document("maHoaDon", maHoaDon).append("maKhuyenMai", maKhuyenMai).append("soTienGiam", soTienGiam));
        return true;
    }

    public HoaDon getByMaHoaDon(String maHoaDon) { return hoaDon(one("HoaDon", "maHoaDon", maHoaDon)); }
