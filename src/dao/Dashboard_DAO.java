...
public class Dashboard_DAO extends MongoDaoSupport {
    ...
    public double getTongDoanhThuTheoNgay(LocalDate ngay) {
        double total = 0;
        for (Document hd : docs("HoaDon")) if (sameDay(hd.get("ngayTao"), ngay)) total += invoiceTotal(s(hd, "maHoaDon"));
        return total;
    }
    ...
    public Map<String, Double> getDoanhThuHomNayTheoGio() {
        ...
        for (Document hd : docs("HoaDon")) {
            if (!sameDay(hd.get("ngayTao"), LocalDate.now())) continue;
    ...
    private int getLuotDatBanTheoNgay(LocalDate ngay) {
        int c = 0; for (Document p : docs("PhieuDatBan")) if (sameDay(p.get("thoiGianBatDau"), ngay)) c++; return c;
    }
    ...
    private int getSoMonBanRaTheoNgay(LocalDate ngay) {
        int c = 0;
        for (Document hd : docs("HoaDon")) if (sameDay(hd.get("ngayTao"), ngay))
            for (Document ct : docs("ChiTietHoaDon", com.mongodb.client.model.Filters.eq("maHoaDon", s(hd, "maHoaDon")))) c += integer(ct, "soLuong");
        return c;
    }

    public Map<String, Integer> getTopMonBanChayHomNay(int limit) { return topMon(LocalDate.now(), null, limit); }
    public Map<String, Integer> getTopMonBanChayTheoThang(int limit) { return topMon(LocalDate.now().withDayOfMonth(1), LocalDate.now(), limit); }
    ...
    private int getSoKhachPhucVuTheoNgay(LocalDate ngay) {
        ...
        for (Document hd : docs("HoaDon")) if (sameDay(hd.get("ngayTao"), ngay) && s(hd, "maKhachHang") != null) set.add(s(hd, "maKhachHang"));
        return set.size();
    }
    ...
    public Map<String, Integer> getTopMonBanChay(int limit) { return topMon(null, null, limit); }
    ...
    public Map<String, Integer> getLuongKhachTheoKhungGio() { return getLuongKhachTheoKhungGio(LocalDate.now()); }
    public Map<String, Integer> getLuongKhachTheoKhungGio(LocalDate ngay) {
        Map<String, Integer> map = new LinkedHashMap<>();
        for (Document p : docs("PhieuDatBan")) if (sameDay(p.get("thoiGianBatDau"), ngay)) {
    ...
    public int getSoKhachMoi() { return (int) col("KhachHang").countDocuments(); }
    public int getSoKhachQuayLai() {
        int c = 0; for (KhachHang k : new KhachHang_DAO().getAllKhachHang()) if (k.getDiemTichLuy() > 0) c++; return c;
    }
    public String tinhPhanTramThayDoi(double homNay, double homQua) {
        if (homQua == 0) return homNay == 0 ? "0%" : "100%";
    ...
    private Map<String, Integer> topMon(LocalDate from, LocalDate to, int limit) {
        ...
        for (Document hd : docs("HoaDon")) {
            ...
            for (Document ct : docs("ChiTietHoaDon", com.mongodb.client.model.Filters.eq("maHoaDon", s(hd, "maHoaDon")))) {
